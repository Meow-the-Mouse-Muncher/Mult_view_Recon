...
class LFNR(nn.Module):
    ...
    def _get_query(self, rays_o, rays_d):
        ...
        return q_enc
    def _get_key(self, projected_rgb_and_feat,  pts_3d):
    ...
    def _predict_color(self, input_k):
        """
    ...
    def forward(self, batch):
        ...
        projected_rgb_and_feat = self._get_pixel_projection(sampling_grid,
                                                    ref_images)
        input_k = self._get_key(projected_rgb_and_feat, pts_3d) # [B, N, n_rays, k_dim] k_dim=130

        rgb,n_attn= self._predict_color(input_k)
