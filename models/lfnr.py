...
class LFNR(vanilla_nlf.VanillaNLF):
  ...
  def _get_key(self, projected_rays, projected_rgb_and_feat, wcoords, ref_idx):
    ...
    Args:
      ...
      ref_idx: The index of the reference views being used for interpolation. #其实就是相机序号
    ...
    # Add the 3d woord coordinates.   进行一个码的编
  ...
  # _, input_k, _, learned_embedding mask没用哈哈
  ...
  def _get_avg_features(self, input_q, input_k, randomized):
    ...
    # 使用一个Transformer进行聚合，得到参考视图特征 out (1,10,65,256)
    out = self.epipolar_transformer(
        input_k,
        deterministic=not randomized,
    )
    refined_query = out[Ellipsis, 0:1, :]  # Get refined query (1,10,1,256)
  ...
  def __call__(self, rng_0, rng_1, batch, randomized):  # 前向传播
    """Light Field Neural Rendering Model.
    ...
    # 获取投影坐标和世界空间坐标 wcoords (1,64,3) projected_coordinates (10,1,64,2)
    # 10 张参考图像 射线采样64个点
    ...
    # projected_rays.origins.shape (1,10,64,3)
    ref_images = model_utils.uint2float(batch.reference_views.rgb)
    # (N,H,W,3) 参考视角的图像
    # 这里首先是对参考视角的图像进行采样获得RGB值，其次是获取cnn特征
    ...
    # input_q (1,3)->（1,36）
    _, input_q, _ = self._get_query(batch_rays)  #对于目标光线进行光场编码 我超，真的麻烦啊。
    # 将参考图像特征拼接并融合编码 input_k (1,10,64,220)  learned_embedding (1,10,64,110)
    _, input_k, _, learned_embedding = self._get_key(projected_rays,
                                                     projected_rgb_and_feat,
                                                     wcoords,
                                                     batch.reference_views.idx)
    # 现在我们可以得到了需要的特征了：
    # input_q 目标光场编码特征 (1,36)
    # input_k 参考视角下的 光场编码特征||世界坐标编码||rgb和cnn特征||相机嵌入特征 (1,10,64,220)

    # Get the average feature over each epipolar line
    #avg_projection_features (1,10,256)  epipolar_attn_weights (1,10,64,1)
